...
Class PraadmpRepository
{
    public function getById($id)
    {
        $data = DB::table('m_praadmp')
            ...
            ->first(['m_praadmp.*', 'users.name', 'users.jabatan', 'users.departemen', 'users.divisi', 'users.distrik']);
    ...
    public function getAllWithUsername()
    {
        $data = DB::table('m_praadmp')
            ->join('users', 'm_praadmp.nrp', '=', 'users.nrp')
            ->select('m_praadmp.*', 'users.name as username','users.departemen as departemen', 'users.divisi as divisi', 'users.distrik as distrik') 
            ->orderBy('m_praadmp.create_at', 'desc')
            ->get();
