...
Class PrapelatihanRepository
{
    public function getById($id)
    {
        $data = DB::table('m_prapelatihan')
            ...
            ->first(['m_prapelatihan.*', 'users.name', 'users.jabatan', 'users.departemen', 'users.divisi', 'users.distrik']);
    ...
    public function getAllWithUsername()
    {
        $data = DB::table('m_prapelatihan')
            ->join('users', 'm_prapelatihan.nrp', '=', 'users.nrp')
            ->select('m_prapelatihan.*', 'users.name as username','users.departemen as departemen', 'users.divisi as divisi',  'users.distrik as distrik') 
            ->orderBy('m_prapelatihan.create_at', 'desc')
            ->get();
